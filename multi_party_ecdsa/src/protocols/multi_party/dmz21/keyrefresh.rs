...
impl KeyRefreshPhase {
    pub fn new(
        party_index: usize,
        private_key_old_string: Option<String>,
        params: Parameters,
        threshold_set: Vec<usize>,
        public_key_old_string: String,
        ...
        if let Some(private_key_old_string) = private_key_old_string {
            private_key_old = Some(
                serde_json::from_str(&private_key_old_string)
                    .map_err(|_| MulEcdsaError::FromStringFailed)?,
            );
        }
        let public_key_old: PublicKey = serde_json::from_str(&public_key_old_string)
            .map_err(|_| MulEcdsaError::FromStringFailed)?;
        // Generate cl keypair
        ...
        msgs.phase_one_msgs
            .insert(party_index, phase_one_msg.clone());
        ...
        if threshold_set.contains(&party_index) {
            let (vss_scheme, secret_shares) = VerifiableSS::<GE>::share(
                params.threshold,
                params.share_count,
                &private_key_old.clone().unwrap().share_sk,
            );
            for i in 0..params.share_count {
                ...
                if i == party_index {
                    ...
                    let lambda = VerifiableSS::<GE>::map_share_to_new_params(
                        &params,
                        party_index,
                        &threshold_set,
                    );
                    ...
                    let phase_two_message = ReceivingMessages::KeyRefreshMessage(
                        KeyRefreshMessage::PhaseTwoMsg(phase_two_msg),
                    );
    ...
    fn handle_phase_two_msg(
        &mut self,
        index: usize,
        msg: &KeyRefreshPhaseTwoMsg,
    ) -> Result<(), MulEcdsaError> {
        //Check VSS
        //Check polynomial constant item equals to old key share
        let share_public_key_old = self
            .public_key_old
            .share_pks
            .get(&index)
        ...
        let lambda =
            VerifiableSS::<GE>::map_share_to_new_params(&self.params, index, &self.threshold_set);
        ...
        match msg {
            ...
            KeyRefreshMessage::PhaseTwoMsg(msg) => {
                ...
                if self.msgs.phase_two_msgs.len() == self.threshold_set.len() {
                    ...
                    let sending_msg = ReceivingMessages::KeyRefreshMessage(
                        KeyRefreshMessage::PhaseThreeMsg(msg_three),
                    );
...
fn test_key_refresh() {
    ...
    //init key refresh
    let mut keyrefresh_0 = KeyRefreshPhase::new(
        0,
        Some(key_old_json_0.to_string()),
        params.clone(),
        threshold_set.clone(),
        public_key_old.to_string(),
    )
    .unwrap();
    let mut keyrefresh_1 = KeyRefreshPhase::new(
        1,
        Some(key_old_json_1.to_string()),
        params.clone(),
        threshold_set.clone(),
        public_key_old.to_string(),
    )
    .unwrap();
    let mut keyrefresh_2 = KeyRefreshPhase::new(
        2,
        None,
        params,
        threshold_set.clone(),
    ...
    let phase_one_msg_0 = keyrefresh_0
        .msgs
        .phase_one_msgs
        .get(&keyrefresh_0.party_index)
        .ok_or(MulEcdsaError::GetIndexFailed)
        .unwrap();
    let phase_one_msg_1 = keyrefresh_1
        .msgs
        .phase_one_msgs
        .get(&keyrefresh_1.party_index)
        .ok_or(MulEcdsaError::GetIndexFailed)
        .unwrap();
    let phase_one_msg_2 = keyrefresh_2
        .msgs
        .phase_one_msgs
    ...
    //party 0
    keyrefresh_0
        .handle_phase_one_msg(
            &phase_one_msg_1.h_caret,
            &phase_one_msg_1.h,
            &phase_one_msg_1.gp,
        )
        .unwrap();
    ...
    for (key, value) in phase_two_msg_0 {
        ...
        match received_msg {
            ReceivingMessages::KeyRefreshMessage(msg_1) => match msg_1 {
                KeyRefreshMessage::PhaseTwoMsg(msg) => {
    ...
    //party 0
    keyrefresh_0
        .handle_phase_two_msg(1, phase_two_msg_1_received.get(&(0 as usize)).unwrap())
        .unwrap();
    keyrefresh_0.msgs.phase_two_msgs.insert(
        1,
        phase_two_msg_1_received.get(&(0 as usize)).unwrap().clone(),
    );
    let phase_three_msg_0 = keyrefresh_0.generate_phase_three_msg();
    keyrefresh_0
        .msgs
        .phase_three_msgs
        .insert(keyrefresh_0.party_index, phase_three_msg_0.clone());
    //party 1
    keyrefresh_1
        .handle_phase_two_msg(0, phase_two_msg_0_received.get(&(1 as usize)).unwrap())
        .unwrap();
    keyrefresh_1.msgs.phase_two_msgs.insert(
        0,
        phase_two_msg_0_received.get(&(1 as usize)).unwrap().clone(),
    );
    let phase_three_msg_1 = keyrefresh_1.generate_phase_three_msg();
    keyrefresh_1
        .msgs
        .phase_three_msgs
        .insert(keyrefresh_0.party_index, phase_three_msg_1.clone());
    //party 2
    keyrefresh_2
        .handle_phase_two_msg(0, phase_two_msg_0_received.get(&(2 as usize)).unwrap())
        .unwrap();
    keyrefresh_2.msgs.phase_two_msgs.insert(
        0,
        phase_two_msg_0_received.get(&(2 as usize)).unwrap().clone(),
    );
    keyrefresh_2
        .handle_phase_two_msg(1, phase_two_msg_1_received.get(&(2 as usize)).unwrap())
        .unwrap();
    keyrefresh_2.msgs.phase_two_msgs.insert(
        1,
        phase_two_msg_1_received.get(&(2 as usize)).unwrap().clone(),
    );
    let phase_three_msg_2 = keyrefresh_2.generate_phase_three_msg();
    keyrefresh_2
        .msgs
        .phase_three_msgs
        .insert(keyrefresh_2.party_index, phase_three_msg_2.clone());

    //handle phase three msgs and generate new key
    //party 0
    keyrefresh_0
        .handle_phase_three_msg(1, &phase_three_msg_1)
        .unwrap();
    keyrefresh_0
        .msgs
        .phase_three_msgs
        .insert(1, phase_three_msg_1.clone());
    keyrefresh_0
        .handle_phase_three_msg(2, &phase_three_msg_2)
        .unwrap();
    keyrefresh_0
        .msgs
        .phase_three_msgs
        .insert(2, phase_three_msg_2.clone());
    ...
    let points0 = vec![0, 1, 2]
        ...
        .collect::<Vec<FE>>();
    let master_key_1 = VerifiableSS::<GE>::lagrange_interpolation_at_zero(
        &[points0[0], points0[1]],
        &vec![priv_key_new_0.share_sk, priv_key_new_1.share_sk],
    );
    let master_key_2 = VerifiableSS::<GE>::lagrange_interpolation_at_zero(
        &[points0[1], points0[2]],
        &vec![priv_key_new_1.share_sk, priv_key_new_2.share_sk],
    );
    let master_key_3 = VerifiableSS::<GE>::lagrange_interpolation_at_zero(
        &[points0[0], points0[2]],
        &vec![priv_key_new_0.share_sk, priv_key_new_2.share_sk],
    );
    assert_eq!(
        pub_key_new_0.pk,
        GE::generator().scalar_mul(&master_key_1.get_element())
    );
    assert_eq!(
        pub_key_new_1.pk,
        GE::generator().scalar_mul(&master_key_2.get_element())
    );
    assert_eq!(
        pub_key_new_2.pk,
        GE::generator().scalar_mul(&master_key_3.get_element())
    );
}
