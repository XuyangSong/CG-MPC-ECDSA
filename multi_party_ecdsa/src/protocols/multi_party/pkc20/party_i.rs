use crate::utilities::cl_enc_proof::*;
...
use class_group::primitives::cl_dl_public_setup::{
    decrypt, encrypt, next_probable_prime, CLGroup, Ciphertext as CLCipher, SK as CLSK,
};
...
impl Setup {
    ...
    pub fn phase_one_generate_commitment(&self) -> (SetupPhaseOneMsg, SetupPhaseTwoMsg) {
        ...
        let commitment =
            HashCommitment::create_commitment_with_user_defined_randomness(&self.r, &blind_factor);

        let msg_one = SetupPhaseOneMsg { commitment };
        let msg_two = SetupPhaseTwoMsg {
            open: (self.r.clone(), blind_factor),
        };
        (msg_one, msg_two)
    }

    pub fn phase_two_verify_commitment_and_generate_qtilde(
        &self,
    ...
    pub fn phase_three_generate_gi_and_commitment(
        &mut self,
        group: &CLGroup,
    ) -> (SetupPhaseThreeMsg, SetupPhaseFourMsg) {
        self.t = BigInt::sample_below(&(&group.stilde * BigInt::from(2).pow(40)));
        ...
        let msg_four = SetupPhaseFourMsg {
            open: (self.gi.clone(), blind),
        };
        (msg_three, msg_four)
    }
    ...
    pub fn phase_five_generate_zkpok(&self, group: &CLGroup) -> SetupPhaseFiveMsg {
        let r = BigInt::sample_below(&(&group.stilde * BigInt::from(2).pow(90)));
    ...
    pub fn phase_five_verify_zkpok_and_generate_gq(
        &mut self,
        group: &CLGroup,
...
impl SignPhase {
    ...
    pub fn phase_one_generate_cl_enc_proof_and_com(
        ...
        let (cipher, r) = encrypt(group, cl_keypair.get_public_key(), &self.k);

        let cl_enc_state = CLEncState {
            cipher: cipher,
            cl_pub_key: cl_keypair.cl_pub_key.clone(),
        };
        let promise_wit = CLEncWit { a: self.k, r: r.0 };
    ...
    pub fn phase_two_generate_homo_cipher(
        ...
        for (i, msg) in sign_phase_one_msg_vec.iter().enumerate() {
            ...
            let beta = FE::new_random();
            {
                ...
                // Handle CL cipher.
                let (r_cipher, _r_blind) = encrypt(&group, pk, &zero.sub(&beta.get_element()));
            ...
            let v = FE::new_random();
            {
                ...
                // Handle CL cipher.
                let (r_cipher, _r_blind) = encrypt(&group, pk, &zero.sub(&v.get_element()));
    ...
    pub fn phase_two_decrypt_and_verify(
        ...
        for i in 0..msg_vec.len() {
            ...
            let miu = decrypt(&group, &sk, &msg_vec[i].homocipher_plus);
