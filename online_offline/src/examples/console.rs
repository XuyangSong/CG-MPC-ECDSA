...
impl Console {
    ...
    async fn process_command(&mut self, command: UserCommand) -> Result<(), String> {
        match command {
            ...
            UserCommand::SignOffline => {
                let msg = bincode::serialize(&ReceivingMessages::SignOfflineBegin)
                    .map_err(|why| format!("bincode serialize error: {}", why))?;
                self.node.broadcast(Message(msg.clone())).await;
                self.node.sendself(Message(msg)).await;
            }
            UserCommand::SignOnline(message) => {
                let msg = bincode::serialize(&ReceivingMessages::SignOnlineBegin(message))
                    .map_err(|why| format!("bincode serialize error: {}", why))?;
                self.node.broadcast(Message(msg.clone())).await;
                self.node.sendself(Message(msg)).await;
            }
