...
use multi_party_ecdsa::utilities::promise_sigma::{PromiseProof, PromiseState};
...
impl SignPhase {
    ...
    pub fn process_begin_sign_online(&mut self, index: usize, message_str: String) -> Result<SendingMessages, MulEcdsaError>{
        if index == 1{
            ...
            let cipher = self.online(&message, &c_2).unwrap();

            let msg_send = ReceivingMessages::TwoSignMessagePartyTwo(
                PartyTwoMsg::SignPartyTwoRoundTwoMsg(cipher, t_p, message));
